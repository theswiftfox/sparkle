                                   ...
                                   : SV_Position)
{
	...
	// packing - not working, idk. i'm stupid?
	/*uint4 chalf = f32tof16(albedo);
	output.albedoMR.r = (chalf.r << 16) | chalf.g;
	output.albedoMR.g = (chalf.b << 16) | chalf.a;
	if (materialFeatures & SPARKLE_MAT_PBR == SPARKLE_MAT_PBR) {
		...
		output.albedoMR.b = (mhalf << 16) | rhalf;
		output.albedoMR.a = 0;
	} else {
		float4 specular = specularTexture.Sample(textureSampler, input.uv);
		uint4 shalf = f32tof16(specular);
		output.albedoMR.b = (shalf.r << 16) | shalf.g;
		output.albedoMR.a = (shalf.b << 16) | shalf.a;
	}
