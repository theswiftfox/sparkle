...
[[vk::location(0)]] PS_OUTPUT main(PS_INPUT input, in float4 pos : SV_Position)
{
	PS_OUTPUT output;
	...
	float4 albedo = float4(1.0f); //albedoTexture.Sample(textureSampler, input.uv);
	float4 normal;
