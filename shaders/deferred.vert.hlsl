...
[[vk::binding(0)]] cbuffer ubo {
	float4x4 viewMat;
	float4x4 projectionMat;
}

[[vk::binding(1)]] cbuffer dubo {
	float4x4 modelMat;
	float4x4 normalMat;
}
...
VS_OUTPUT main(in VS_INPUT input, out float4 vtxPos : SV_Position) {
	...
	output.normal = mul(normalMat, normalize(input.normal));
	output.tangent = mul(normalMat, normalize(input.tangent));
