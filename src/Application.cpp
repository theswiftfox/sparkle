...
void App::drawGui(size_t imageIndex, bool renderQueued) {
	if (renderQueued) {
	...
	imguiHandlesMouse = imguiIo.WantCaptureMouse;
	imguiHandlesKeyboard = imguiIo.WantCaptureKeyboard;

	{
		...
		/* FPS */
		if (showFPS) {
			ImGui::SetNextWindowPos(ImVec2(windowData.Width - 100, 10));
			int flags = ImGuiWindowFlags_NoTitleBar;
			flags |= ImGuiWindowFlags_AlwaysAutoResize;
			flags |= ImGuiWindowFlags_NoMove;
	...
	VkSemaphore imageReadySem[] = { renderQueued ? semRenderFinished[frameCounter] : semImageAvailable[frameCounter] };
	VkSemaphore guiDrawnSem[] = { semGuiRendered[frameCounter] };

	ImGui_ImplVulkanH_FrameData* fd = &windowData.Frames[windowData.FrameIndex];

	checkVkResult(vkResetCommandPool(pVulkanDevice, fd->CommandPool, 0));
	...
	clearValue.color = cClearColor;
	VkRenderPassBeginInfo renderInfo = {};
	renderInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
	renderInfo.renderPass = windowData.RenderPass;
	renderInfo.framebuffer = windowData.Framebuffer[imageIndex];
	renderInfo.renderArea.extent.width = windowData.Width;
	renderInfo.renderArea.extent.height = windowData.Height;
	...
	submit.pWaitSemaphores = imageReadySem;
	submit.pWaitDstStageMask = &waitStage;
	submit.commandBufferCount = 1;
	submit.pCommandBuffers = &fd->CommandBuffer;
	submit.signalSemaphoreCount = 1;
	submit.pSignalSemaphores = guiDrawnSem;

	checkVkResult(vkEndCommandBuffer(fd->CommandBuffer));
	checkVkResult(vkQueueSubmit(pGraphicsQueue, 1, &submit, inFlightFences[frameCounter]));
}

void App::drawFrame(double deltaT) {
	vkWaitForFences(pVulkanDevice, 1, &inFlightFences[frameCounter], VK_TRUE, std::numeric_limits<uint64_t>::max());
	vkResetFences(pVulkanDevice, 1, &inFlightFences[frameCounter]);
...
void App::createSwapChain(VkSwapchainKHR oldSwapChain) {
...
void App::createSamplerDescriptorSet() {
...
void App::setupApplicationData(std::string config) {
...
void App::recreateDrawCommandBuffers() {
...
void App::recreateAllCommandBuffers() {
...
void App::increaseDrawBufferSize(VkDeviceSize newVertLimit, VkDeviceSize newIndexLimit) {
...
VkCommandBuffer App::beginOneTimeCommand() const {
	VkCommandBufferAllocateInfo allocInfo = {
		VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
		nullptr,
		pCommandPool,
...
void App::endOneTimeCommand(VkCommandBuffer buffer) const {
...
void App::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, vkExt::Buffer& buffer, vkExt::SharedMemory* bufferMemory) const {
	VkBuffer tmpBuffer;
...
void App::allocateMemory(VkDeviceSize size, VkMemoryPropertyFlags properties, uint32_t memoryTypeFilterBits, vkExt::SharedMemory* memory) const {
...
void App::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, VkCommandBuffer commandBuff /* = nullptr */) const {
...
RequiredQueueFamilyIndices App::getQueueFamilies(VkPhysicalDevice device) const {
...
SwapChainSupportInfo App::getSwapChainSupportInfo(VkPhysicalDevice device) const {
...
VkSurfaceFormatKHR App::getBestSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR App::getBestPresentMode(const std::vector<VkPresentModeKHR>& availableModes) {
...
VkExtent2D App::getSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const {
...
size_t App::checkDeviceRequirements(VkPhysicalDevice device) const {
	size_t score = 0;
...
std::vector<const char*> App::getRequiredExtensions() const {
 ...
 uint32_t App::findMemoryType(uint32_t filter, VkMemoryPropertyFlags properties) const {
