...
void RenderBackend::createMaterialDescriptorSetLayout()
{
    ...
    VK_THROW_ON_ERROR(vkCreateDescriptorSetLayout(pVulkanDevice, &layoutInfo, nullptr, &pMaterialDescriptorSetLayout), "DescriptorSetLayout creation failed!");
}
...
void RenderBackend::createCommandPool()
{
    ...
    VK_THROW_ON_ERROR(vkCreateCommandPool(pVulkanDevice, &cmdPoolInfo, nullptr, &pCommandPool), "CommandPool creation failed!");
}
...
void RenderBackend::setupDebugCallback()
{
    ...
    VK_THROW_ON_ERROR(CreateDebugReportCallbackEXT(pVulkanInstance, &createInfo, nullptr, &pCallback), ("failed to set debug report callback"));
}
...
void RenderBackend::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, VkCommandBuffer commandBuff /* = nullptr */) const
{
    ...
    barrier.subresourceRange.aspectMask = format == depthFormat ? formatHasStencilComponent(format) ? VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT : VK_IMAGE_ASPECT_DEPTH_BIT
