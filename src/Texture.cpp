...
Texture::Texture(std::string filePath) {
	...
	staging.copyBufferToImage(context.pCommandPool, context.pGraphicsQueue, texImage.image, static_cast<uint32_t>(width), static_cast<uint32_t>(height));
	...
	if (vkCreateSampler(context.pVulkanDevice, &samplerInfo, nullptr, &texImageSampler) != VK_SUCCESS) {
...
Texture::~Texture() {
	auto& context = App::getHandle();
	vkDestroySampler(context.pVulkanDevice, texImageSampler, nullptr);
	vkDestroyImageView(context.pVulkanDevice, texImageView, nullptr);
