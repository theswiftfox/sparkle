...
void App::initialize(std::string config)
{
	...
	auto ok = config.empty() ? pSettings->load() : pSettings->load(config);
	if (!ok) {
		std::cout << "Error loading settings" << std::endl;
	}
	windowWidth = pSettings->resolution.width;
	windowHeight = pSettings->resolution.height;

	pCamera = std::make_unique<Camera>(pSettings);
	createWindow();

	pRenderer = std::make_unique<Sparkle::RenderBackend>(pWindow, APP_NAME, pCamera);
	pRenderer->initialize(pSettings, pSettings->validationLayers);

	pSceneLoader = std::make_unique<Import::SceneLoader>();
	pSceneLoader->loadFromFile(pSettings->levelPath);
