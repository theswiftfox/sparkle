...
Sparkle::Material::Material(std::vector<std::shared_ptr<Sparkle::Texture>> textures, bool postponeInit)
{
	uniforms.features = 0x0;
	initTextureMaterial(textures, postponeInit);
}
...
void Sparkle::Material::initTextureMaterial(std::vector<std::shared_ptr<Sparkle::Texture>> textures, bool postponeInit)
{
...
void Sparkle::Material::updateDescriptorSets()
{
	...
	if (textures.find(TEX_TYPE_PLACEHOLDER) == textures.end()) {
		throw std::runtime_error("Missing placeholder texture");
	}
	auto placeholder = textures[TEX_TYPE_PLACEHOLDER]->descriptor();

	{
		auto descriptor = placeholder;
		if (textures.find(TEX_TYPE_DIFFUSE) != textures.end()) {
			descriptor = textures[TEX_TYPE_DIFFUSE]->descriptor();
		}
	...
	}
	{
		...
		if (textures.find(TEX_TYPE_SPECULAR) != textures.end()) {
			descriptor = textures[TEX_TYPE_SPECULAR]->descriptor();
		}
		VkWriteDescriptorSet write = {
			VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
			nullptr,
	...
	}
	{
		...
		} else if (textures.find(TEX_TYPE_METALLIC_ROUGHNESS) != textures.end()) {
			descriptor = textures[TEX_TYPE_METALLIC_ROUGHNESS]->descriptor();
			uniforms.features |= SPARKLE_MAT_METALLIC_ROUGHNESS;
		}
