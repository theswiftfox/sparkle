...
Camera::Camera(std::shared_ptr<Config> settings)
    : cameraPos(0.0f)
{
	auto width = settings->resolution.width, height = settings->resolution.height;
	nearZ = NEAR_Z;
	farZ = settings->renderDistance;
	projMat = glm::perspective(glm::radians(settings->fov), (float)width / (float)height, nearZ, farZ);
	projMat[1, 1] *= -1.0f;
...
{
	projMat = glm::perspective(glm::radians(gameSettings->fov), aspect, NEAR_Z, gameSettings->renderDistance);
}

void Camera::resetAspect()
{
	auto width = gameSettings->resolution.width, height = gameSettings->resolution.height;
	projMat = glm::perspective(glm::radians(gameSettings->fov), (float)width / (float)height, NEAR_Z, gameSettings->renderDistance);
}
